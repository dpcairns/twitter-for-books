...
test('front page lets us filter down books', async ({ page}) => {
  ...
  await expect(page).toHaveScreenshot('front-page-moby.png', { maxDiffPixelRatio: .03, fullPage: true });
  ...
  await page.waitForLoadState('networkidle');
  await expect(page).toHaveScreenshot('detail-page-moby-1.png', { maxDiffPixelRatio: .00000003, fullPage: true });
});
