...
test('front page lets us filter down books', async ({ page}) => {
  ...
  await expect(page).toHaveScreenshot('front-page-moby.png', { maxDiffPixels: 100 });
  ...
  await expect(page).toHaveScreenshot('detail-page-moby-1.png',{ maxDiffPixels: 6000 });
  ...
  await page.waitForLoadState('networkidle');
  await expect(page).toHaveScreenshot('detail-page-moby-1.png', { maxDiffPixels: 10000 });
});
...
test('detail page loads tweets two at a time on click', async ({ page }) => {
  ...
  await expect(page).toHaveScreenshot('detail-page-forster-1.png', { maxDiffPixels: 9000 });
